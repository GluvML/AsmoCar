...
class App {
    ...
    init() {
        ...
        // 0. Initialize Font Theme
        const savedFont = localStorage.getItem('co2app_font') || 'plus-jakarta-sans';
    ...
    translateStaticUI() {
        ...
        // Time tabs in charts
        const timeTabRealtime = document.querySelector('.time-tab[data-range="realtime"]');
        if (timeTabRealtime) timeTabRealtime.innerHTML = `<span class="live-dot"></span>${window.i18n.t('range_realtime')}`;
    ...
    renderDashboard() {
        ...
        // Update Cockpit Emergency Alert Banner & Screen Danger Aura
        this.updateEmergencyAlertUI(data, thresholds);
    }

    updateEmergencyAlertUI(data, thresholds) {
        if (!data) return;

        const isCo2Danger = data.co2 > thresholds.co2.max;
        const isTempDanger = data.temp > thresholds.temp.max + 3 || data.temp < thresholds.temp.min - 3;
        const isHumDanger = data.humidity > thresholds.humidity.max + 10 || data.humidity < thresholds.humidity.min - 10;
        const isDanger = isCo2Danger || isTempDanger || isHumDanger;

        // 1. Viewport danger perimeter pulse
        const viewport = document.getElementById('screens-viewport');
        if (viewport) {
            viewport.classList.toggle('has-emergency-alert', isDanger);
        }

        // 2. Cockpit Emergency Alert Banner
        const banner = document.getElementById('cockpit-emergency-banner');
        if (banner) {
            if (isDanger && !this._emergencyDismissed) {
                banner.style.display = 'flex';
                const titleEl = document.getElementById('emergency-banner-title');
                const valEl = document.getElementById('emergency-banner-val');
                const descEl = document.getElementById('emergency-banner-desc');

                if (isCo2Danger) {
                    if (titleEl) titleEl.textContent = 'CẢNH BÁO: NỒNG ĐỘ CO₂ VƯỢT NGƯỠNG AN TOÀN!';
                    if (valEl) valEl.textContent = `${Math.round(data.co2)} ppm`;
                    if (descEl) descEl.textContent = 'Tài xế hãy chuyển ngay sang LẤY GIÓ NGOÀI hoặc HẠ KÍNH XE để tránh buồn ngủ và thiếu oxy!';
                } else if (isTempDanger) {
                    if (titleEl) titleEl.textContent = 'CẢNH BÁO: NHIỆT ĐỘ CABIN VƯỢT MỨC CHO PHÉP!';
                    if (valEl) valEl.textContent = `${data.temp.toFixed(1)}°C`;
                    if (descEl) descEl.textContent = 'Nhiệt độ buồng lái bất thường. Vui lòng điều chỉnh lại hệ thống điều hòa!';
                } else {
                    if (titleEl) titleEl.textContent = 'CẢNH BÁO: ĐỘ ẨM CABIN BẤT THƯỜNG!';
                    if (valEl) valEl.textContent = `${Math.round(data.humidity)}%`;
                    if (descEl) descEl.textContent = 'Độ ẩm vượt ngưỡng, nguy cơ đọng sương làm mờ kính lái. Hãy bật sấy kính (Defrost)!';
                }
            } else {
                banner.style.display = 'none';
                if (!isDanger) {
                    this._emergencyDismissed = false; // Reset dismiss when air returns to safe range
                }
            }
        }

        // 3. Dismiss button handler
        const dismissBtn = document.getElementById('emergency-banner-action');
        if (dismissBtn && !dismissBtn._hasListener) {
            dismissBtn._hasListener = true;
            dismissBtn.addEventListener('click', () => {
                this._emergencyDismissed = true;
    ...
    startRealtimeUpdates() {
        ...
        this.updateTimer = setInterval(() => {
            ...
            if (this.currentScreen === 'dashboard') {
                ...
                if (window.ChartManager) {
                    window.ChartManager.updateLiveReading(currentReading);
                }
    ...
    onHardwareData(co2, temp, hum, source = 'BLE') {
        ...
        if (this.currentScreen === 'dashboard') {
            ...
            if (window.ChartManager) {
                window.ChartManager.updateLiveReading(reading);
            }
        } else if (this.currentScreen === 'alerts') {
            window.AlertManager.renderAlertsList();
        }
