...
class ChartManager {
    ...
    init() {
        ...
        // Auto register ChartStreaming if loaded
        if (typeof ChartStreaming !== 'undefined') {
            try { Chart.register(ChartStreaming); } catch (e) {}
        }
    ...
    // ----------------------------------------------------------
    // Line Chart (Live Telemetry & Historical Trends)
    // ----------------------------------------------------------
    renderLineChart(canvasId, roomId, timeRange) {
        ...
        const history = window.DataManager.getHistory(roomId, range === 'realtime' ? '24h' : range);
        ...
        if (isRealtime) {
            ...
            const recent = history.filter(d => d.timestamp >= cutoff5m);
            if (recent.length >= 15) {
                sampled = recent;
            } else {
                // Synthesize smooth high-resolution live wave for the 5m window ending at current reading
                const cur = history[history.length - 1] || { co2: 520, temp: 25.0, humidity: 55.0 };
                const points = 25;
                const step = 300000 / points; // 12 seconds per point
                sampled = [];
                for (let i = points; i >= 0; i--) {
                    const t = now - (i * step);
                    // Smooth envelope that converges to 0 at i=0 so it seamlessly matches cur
                    const env = Math.min(1, i / 5);
                    const angle = i * 0.28;
                    const cWave = Math.round(cur.co2 + (Math.sin(angle) * 14 + Math.cos(angle * 0.7) * 8) * env);
                    const tWave = Math.round((cur.temp + Math.sin(angle * 0.5) * 0.2 * env) * 10) / 10;
                    const hWave = Math.round((cur.humidity + Math.cos(angle * 0.6) * 0.8 * env) * 10) / 10;
                    sampled.push({
                        timestamp: t,
                        co2: cWave,
                        temp: tWave,
            ...
            sampled = this._downsample(history, maxPoints);
        }
        ...
        if (this.activeMetrics.has('co2')) {
            datasets.push({
                ...
                data: sampled.map(d => ({ x: d.timestamp, y: d.co2 })),
                borderColor: isLight ? '#0d9488' : '#14b8a6',
                backgroundColor: isLight ? 'rgba(13, 148, 136, 0.08)' : 'rgba(20, 184, 166, 0.12)',
                fill: true,
                tension: 0.4,
                pointRadius: 0,
                pointHoverRadius: 4,
                pointHoverBackgroundColor: isLight ? '#0d9488' : '#14b8a6',
                borderWidth: 2,
                yAxisID: 'y-co2',
                order: 1
            });
        ...
        if (this.activeMetrics.has('temp')) {
            datasets.push({
                ...
                data: sampled.map(d => ({ x: d.timestamp, y: unitSetting === 'F' ? (d.temp * 9 / 5) + 32 : d.temp })),
                borderColor: isLight ? '#d97706' : '#f59e0b',
                backgroundColor: isLight ? 'rgba(217, 119, 6, 0.08)' : 'rgba(245, 158, 11, 0.12)',
                fill: true,
                tension: 0.4,
                pointRadius: 0,
                pointHoverRadius: 4,
        ...
        if (this.activeMetrics.has('humidity')) {
            datasets.push({
                ...
                data: sampled.map(d => ({ x: d.timestamp, y: d.humidity })),
                borderColor: isLight ? '#0284c7' : '#0ea5e9',
                backgroundColor: isLight ? 'rgba(2, 132, 199, 0.08)' : 'rgba(14, 165, 233, 0.12)',
                fill: true,
                tension: 0.4,
                pointRadius: 0,
                pointHoverRadius: 4,
                pointHoverBackgroundColor: isLight ? '#0284c7' : '#0ea5e9',
                borderWidth: 2,
                yAxisID: 'y-temp',
                order: 3
            });
        ...
        const xScaleConfig = isRealtime ? {
            type: 'realtime',
            realtime: {
                duration: 300000, // 5 phút hiển thị sóng trôi sang trái mượt mà
                refresh: 1000,
                delay: 1000,
                frameRate: 30 // 30 FPS trôi đều sang trái
            },
        ...
        const config = {
            ...
            options: {
                    ...
                    tooltip: {
                        ...
                        callbacks: {
                            title: (items) => {
                                ...
                                const rawX = items[0].raw?.x || items[0].parsed?.x;
                                const d = new Date(rawX || Date.now());
                                const locale = window.i18n.lang === 'vi' ? 'vi-VN' : window.i18n.lang === 'zh' ? 'zh-CN' : 'en-US';
                                return d.toLocaleString(locale, {
                                    day: '2-digit', month: '2-digit',
                                    hour: '2-digit', minute: '2-digit',
                                    second: isRealtime ? '2-digit' : undefined
                                });
                            }
                        }
                    }
                ...
                animation: isRealtime ? { duration: 0 } : {
                    duration: 500,
                    easing: 'easeOutQuart'
                }
            }
    ...
    // ----------------------------------------------------------
    // Live Reading Update with Smooth Leftward Scroll Animation
    // ----------------------------------------------------------
    updateLiveReading(reading) {
        ...
        const unitSetting = window.DataManager.getSettings().unit || 'C';
        const tempVal = unitSetting === 'F' ? (reading.temp * 9 / 5) + 32 : reading.temp;

        if (this.activeRange === 'realtime') {
            // Chế độ TRỰC TIẾP: Đẩy dữ liệu mới vào datasets, chartjs-plugin-streaming tự động cuộn dải sóng sang TRÁI liên tục
            this.lineChart.data.datasets.forEach(ds => {
                if (ds.metricKey === 'co2') {
                    ds.data.push({ x: reading.timestamp, y: reading.co2 });
                } else if (ds.metricKey === 'temp') {
                    ds.data.push({ x: reading.timestamp, y: tempVal });
                } else if (ds.metricKey === 'humidity') {
                    ds.data.push({ x: reading.timestamp, y: reading.humidity });
                }
            });
            // chartjs-plugin-streaming renders automatically on animation frames
        } else {
            // Chế độ 24 giờ / Lịch sử: Hiệu ứng trôi sang TRÁI (Hardware-accelerated Slide Left)
            const wrapper = canvas.closest('.chart-wrapper');
            if (wrapper && !this._isSliding) {
                this._isSliding = true;
                wrapper.classList.add('sliding-left');

                setTimeout(() => {
                    wrapper.classList.remove('sliding-left');
                    this._isSliding = false;
                ...
                datasets: [
                    {
                        ...
                        backgroundColor: isLight ? 'rgba(13, 148, 136, 0.75)' : 'rgba(20, 184, 166, 0.75)',
                        borderColor: isLight ? '#0d9488' : '#14b8a6',
                        borderWidth: 1,
                        borderRadius: 6,
                        yAxisID: 'y-co2'
                    ...
                    },
                    {
                        ...
                        backgroundColor: isLight ? 'rgba(2, 132, 199, 0.75)' : 'rgba(14, 165, 233, 0.75)',
                        borderColor: isLight ? '#0284c7' : '#0ea5e9',
                        borderWidth: 1,
                        borderRadius: 6,
                        yAxisID: 'y-temp'
    ...
    // ----------------------------------------------------------
    updateStats() {
        const range = this.activeRange === 'realtime' ? '24h' : this.activeRange;
        const stats = window.DataManager.getStats(window.DataManager.currentRoomId, range);
    ...
    _downsample(data, maxPoints) {
        ...
        // Always include last point, ensuring no micro-gap vertical cliff
        const last = data[data.length - 1];
        if (result.length > 0 && result[result.length - 1].timestamp !== last.timestamp) {
            if (last.timestamp - result[result.length - 1].timestamp < 30000) {
